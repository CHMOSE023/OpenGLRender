...
bool ModelStd::Load(const char* fileName)  // 加载模型
{
	size_t  length = 0;
	char* xmlData = ReadFile(fileName, length);
	if (xmlData == 0)
...
{	
	m_TimeIndex += (fElapsed  * 30 );    // 每秒30帧

	int timeIndex = (int)m_TimeIndex;
	int index = timeIndex % m_KeyMat.size(); // 取模数组不越界
	...
	// 设置模型矩阵
	glm::mat4 model      = keyMat;                   // ！！！使用帧矩阵赋值	
	model                = glm::scale(model,glm::vec3(3,3,3)); // 缩小0.1
