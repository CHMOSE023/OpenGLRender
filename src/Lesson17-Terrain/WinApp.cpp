...
void WinApp::Initialize(int width, int height,const char*title)
{
	...
	// 初始化raw灰度图地形
	m_terrain.SetShader(m_ShaderDirLight);
	m_terrain.LoadTerrain("textures/Terrain.raw", 1024, 1024);
...
{	
	...
	glm::mat4 model = glm::translate(glm::mat4(1), glm::vec3(0,50,0));

	// 绘制地面
	m_terrain.Render(m_ThirdCamera, model);
